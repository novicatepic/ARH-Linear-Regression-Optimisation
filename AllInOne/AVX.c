...
int main(int argc, char *argv[])
{
    ...
    if(fp != NULL) {
        ...
        for(int i = 0; i < partialIterations; i++) {
            ...
            tmp1.m256d = _mm256_set_pd(xValues[i], xValues[i+1], xValues[i+2], xValues[i+3]);
            tmp2.m256d = _mm256_set_pd(yValues[i], yValues[i+1], yValues[i+2], yValues[i+3]);
            tmp3.m256d = _mm256_set_pd(xValues[i]*xValues[i], xValues[i+1]*xValues[i+1], xValues[i+2]*xValues[i+2], xValues[i+3]*xValues[i+3]);
            tmp4.m256d = _mm256_set_pd(xValues[i]*yValues[i], xValues[i+1]*yValues[i+1], xValues[i+2]*yValues[i+2], xValues[i+3]*yValues[i+3]);
        ...
        for(int i = 0; i < leftoverIterations; i++) {
            tmp1.m256d = _mm256_set_pd(0.0, 0.0, 0.0, xValues[numElements-i-1]);
            tmp2.m256d = _mm256_set_pd(0.0, 0.0, 0.0, yValues[numElements-i-1]);
            tmp3.m256d = _mm256_set_pd(0.0, 0.0, 0.0, xValues[numElements-i-1]*xValues[numElements-i-1]);
            tmp4.m256d = _mm256_set_pd(0.0, 0.0, 0.0, xValues[numElements-i-1]*yValues[numElements-i-1]);
            rX.m256d = _mm256_add_pd(rX.m256d, tmp1.m256d);
            rY.m256d = _mm256_add_pd(rY.m256d, tmp2.m256d);
            rXtimesX.m256d = _mm256_add_pd(rXtimesX.m256d, tmp3.m256d);
            rXtimesY.m256d = _mm256_add_pd(rXtimesY.m256d, tmp4.m256d);
