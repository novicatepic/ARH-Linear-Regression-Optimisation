...
int main(int argc, char *argv[])
{
    ...
    if(fp != NULL) {
        ...
        int leftoverPartials = partialIterations % 4;
        int mulLeftovers = leftoverIterations * 4;
        int fullAdder = mulLeftovers + leftoverIterations;
        ...
        #pragma omp parallel private(i) reduction(+:sumX, sumY, sumXSquare, sumXmultiplY) num_threads(4) 
        {
	        ...
	        {
                tmp1.m256d = _mm256_loadu_pd(xValues+j*4);
                rX.m256d = _mm256_add_pd(rX.m256d, tmp1.m256d);
                tmp2.m256d = _mm256_loadu_pd(yValues+j*4);
                ...
                tmp4.m256d = _mm256_loadu_pd(xValues+j*4);
        ...
        for(int k = 0; k < fullAdder; k++)
        {
            int elemX = xValues[numElements-k-1];
            int elemY = yValues[numElements-k-1];
            sumX += elemX;
            sumY += elemY;
            sumXSquare += elemX*elemX;
            sumXmultiplY += elemX*elemY;
        }
